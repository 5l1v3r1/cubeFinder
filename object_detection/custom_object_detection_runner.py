...
def detect_objects(image_path):
    ...
    """
    print("Writing to original")
    cv_image = cv2.imread(image_path)
    cv2.imwrite('original/{}'.format(image_path), cv_image)
    """
    ...
    # cv_image = cv2.cvtColor(image_np, cv2.COLOR_GRAY2BGR)
    print("Showing image")
    cv2.imwrite('output/{}'.format(image_path), image_np)
    cv2.imshow('Image-{}'.format(image_path), image_np)
...
with detection_graph.as_default():
    with tf.Session(graph=detection_graph) as sess:
        ...
        for image_path in TEST_IMAGE_PATHS:
            ...
            print("Executed " + str(image_path) + " " + str(end - start) + " seconds")
